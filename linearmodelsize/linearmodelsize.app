...
var newa := B{ }
	init {
		...
		newa.setProps(defstring);
	}
	
	par{"size " output(newa.toJSON().toString().length()) " b " output(newa.toJSON().toString().length().floatValue()/1024.floatValue()) " kb"}
	par{"count " output(select count(*) from B) }
	}
