          ...
          else
          {
            ...
            else
            {
              if ( kind == "C" )
              {
                if ( C.mayCreateSynchronize() )
                {
                  for ( count : Int from 0 to entities.length() )
                    {
                      var localErrors := JSONArray() ;
                      var entity := C{id := entities.getJSONObject(count).getString("id").parseUUID()} ;
                      if ( entity != null )
                      {
                        entity.save();
                      }
              ...
              else
              {
                if ( kind == "D" )
                {
                  if ( D.mayCreateSynchronize() )
                  {
                    for ( count : Int from 0 to entities.length() )
                      {
                        var localErrors := JSONArray() ;
                        var entity := D{id := entities.getJSONObject(count).getString("id").parseUUID()} ;
                        if ( entity != null )
                        {
                          entity.save();
                        }
                  ...
                  {
                    errors.put("not allowed to create D");
                  }
                }
                else
                {
                  if ( kind == "E" )
                  {
                    if ( E.mayCreateSynchronize() )
                    {
                      for ( count : Int from 0 to entities.length() )
                        {
                          var localErrors := JSONArray() ;
                          var entity := E{id := entities.getJSONObject(count).getString("id").parseUUID()} ;
                          if ( entity != null )
                          {
                            entity.save();
                          }
                    ...
                    {
                      errors.put("not allowed to create E");
                    }
                  }
                  else
                  {
                    if ( kind == "F" )
                    {
                      if ( F.mayCreateSynchronize() )
                      {
                        for ( count : Int from 0 to entities.length() )
                          {
                            var localErrors := JSONArray() ;
                            var entity := F{id := entities.getJSONObject(count).getString("id").parseUUID()} ;
                            if ( entity != null )
                            {
                              entity.save();
                            }
                      ...
                      {
                        errors.put("not allowed to create F");
                      }
                    }
                    else
                    {
                    ...
                    for ( count : Int from 0 to entities.length() )
                      {
                        ...
                        {
                          mapperNewD(entity, entities.getJSONObject(count), localErrors);
                          var exceptions := entity.validateSave() ;
                          addValidateExceptionsToErrors(exceptions, localErrors);
                        }
                        if ( localErrors.length() > 0 )
                      ...
                      for ( count : Int from 0 to entities.length() )
                        {
                          ...
                          {
                            mapperNewE(entity, entities.getJSONObject(count), localErrors);
                            var exceptions := entity.validateSave() ;
                            addValidateExceptionsToErrors(exceptions, localErrors);
                          }
                          if ( localErrors.length() > 0 )
                        ...
                        for ( count : Int from 0 to entities.length() )
                          {
                            ...
                            {
                              mapperNewF(entity, entities.getJSONObject(count), localErrors);
                              var exceptions := entity.validateSave() ;
                              addValidateExceptionsToErrors(exceptions, localErrors);
                            }
                            if ( localErrors.length() > 0 )
