module webservices/mappers/E
function mapperEditedE ( ent : E , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("F", obj.getString("id").parseUUID()) as F ) ;
            if ( localent != null )
            {
...
function mapperNewE ( ent : E , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedE(ent, json, localerrors);
}