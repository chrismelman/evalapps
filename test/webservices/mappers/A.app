...
function mapperEditedA ( ent : A , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("listb") )
  {
    localerrors.put(makeJSONErrorObject("Entity is missing property listb", "warning"));
  }
  else
  {
    if ( json.get("listb") == json.NULL )
    {
      ent.listb := null;
    }
    else
    {
      var temp := json.getJSONArray("listb") ;
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            {
              localerrors.put(makeJSONErrorObject("Trying to add non existing object for property: listb", "warning"));
            }
          }
        }
      if ( ent.listb != collection )
      {
        ent.listb := collection;
      }
    }
  }
  if ( ! json.has("name") )
