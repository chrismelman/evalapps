module webservices/mappers/A3
function mapperEditedA3 ( ent : A3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("B3", obj.getString("id").parseUUID()) as B3 ) ;
            if ( localent != null )
            {
...
function mapperNewA3 ( ent : A3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedA3(ent, json, localerrors);
}