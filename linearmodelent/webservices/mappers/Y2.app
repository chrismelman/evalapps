module webservices/mappers/Y2
function mapperEditedY2 ( ent : Y2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("Z2", obj.getString("id").parseUUID()) as Z2 ) ;
            if ( localent != null )
            {
...
function mapperNewY2 ( ent : Y2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedY2(ent, json, localerrors);
}