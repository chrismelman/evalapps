module webservices/mappers/U1
function mapperEditedU1 ( ent : U1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("V1", obj.getString("id").parseUUID()) as V1 ) ;
            if ( localent != null )
            {
...
function mapperNewU1 ( ent : U1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedU1(ent, json, localerrors);
}