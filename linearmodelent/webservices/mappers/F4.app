module webservices/mappers/F4
function mapperEditedF4 ( ent : F4 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("G4", obj.getString("id").parseUUID()) as G4 ) ;
            if ( localent != null )
            {
...
function mapperNewF4 ( ent : F4 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedF4(ent, json, localerrors);
}