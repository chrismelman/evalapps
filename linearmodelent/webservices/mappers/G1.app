module webservices/mappers/G1
function mapperEditedG1 ( ent : G1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("H1", obj.getString("id").parseUUID()) as H1 ) ;
            if ( localent != null )
            {
...
function mapperNewG1 ( ent : G1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedG1(ent, json, localerrors);
}