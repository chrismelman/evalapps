module webservices/mappers/A4
function mapperEditedA4 ( ent : A4 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("B4", obj.getString("id").parseUUID()) as B4 ) ;
            if ( localent != null )
            {
...
function mapperNewA4 ( ent : A4 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedA4(ent, json, localerrors);
}