module webservices/mappers/P
function mapperEditedP ( ent : P , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("Q", obj.getString("id").parseUUID()) as Q ) ;
            if ( localent != null )
            {
...
function mapperNewP ( ent : P , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedP(ent, json, localerrors);
}