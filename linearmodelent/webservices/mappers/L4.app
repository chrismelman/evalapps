module webservices/mappers/L4
function mapperEditedL4 ( ent : L4 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("M4", obj.getString("id").parseUUID()) as M4 ) ;
            if ( localent != null )
            {
...
function mapperNewL4 ( ent : L4 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedL4(ent, json, localerrors);
}