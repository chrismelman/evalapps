module webservices/mappers/N2
function mapperEditedN2 ( ent : N2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("O2", obj.getString("id").parseUUID()) as O2 ) ;
            if ( localent != null )
            {
...
function mapperNewN2 ( ent : N2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedN2(ent, json, localerrors);
}