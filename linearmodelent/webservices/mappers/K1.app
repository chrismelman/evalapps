module webservices/mappers/K1
function mapperEditedK1 ( ent : K1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("L1", obj.getString("id").parseUUID()) as L1 ) ;
            if ( localent != null )
            {
...
function mapperNewK1 ( ent : K1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedK1(ent, json, localerrors);
}