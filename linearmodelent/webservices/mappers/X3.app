module webservices/mappers/X3
function mapperEditedX3 ( ent : X3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("Y3", obj.getString("id").parseUUID()) as Y3 ) ;
            if ( localent != null )
            {
...
function mapperNewX3 ( ent : X3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedX3(ent, json, localerrors);
}