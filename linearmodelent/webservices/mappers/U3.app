module webservices/mappers/U3
function mapperEditedU3 ( ent : U3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("V3", obj.getString("id").parseUUID()) as V3 ) ;
            if ( localent != null )
            {
...
function mapperNewU3 ( ent : U3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedU3(ent, json, localerrors);
}