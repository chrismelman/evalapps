module webservices/mappers/T1
function mapperEditedT1 ( ent : T1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("U1", obj.getString("id").parseUUID()) as U1 ) ;
            if ( localent != null )
            {
...
function mapperNewT1 ( ent : T1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedT1(ent, json, localerrors);
}