module webservices/mappers/L2
function mapperEditedL2 ( ent : L2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("M2", obj.getString("id").parseUUID()) as M2 ) ;
            if ( localent != null )
            {
...
function mapperNewL2 ( ent : L2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedL2(ent, json, localerrors);
}