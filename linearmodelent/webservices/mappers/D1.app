module webservices/mappers/D1
function mapperEditedD1 ( ent : D1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("E1", obj.getString("id").parseUUID()) as E1 ) ;
            if ( localent != null )
            {
...
function mapperNewD1 ( ent : D1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedD1(ent, json, localerrors);
}