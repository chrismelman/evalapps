module webservices/mappers/U
function mapperEditedU ( ent : U , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("V", obj.getString("id").parseUUID()) as V ) ;
            if ( localent != null )
            {
...
function mapperNewU ( ent : U , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedU(ent, json, localerrors);
}