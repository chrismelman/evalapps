module webservices/mappers/N1
function mapperEditedN1 ( ent : N1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("O1", obj.getString("id").parseUUID()) as O1 ) ;
            if ( localent != null )
            {
...
function mapperNewN1 ( ent : N1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedN1(ent, json, localerrors);
}