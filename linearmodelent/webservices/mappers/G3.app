module webservices/mappers/G3
function mapperEditedG3 ( ent : G3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("H3", obj.getString("id").parseUUID()) as H3 ) ;
            if ( localent != null )
            {
...
function mapperNewG3 ( ent : G3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedG3(ent, json, localerrors);
}