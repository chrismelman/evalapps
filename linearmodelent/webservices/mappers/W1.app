module webservices/mappers/W1
function mapperEditedW1 ( ent : W1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("X1", obj.getString("id").parseUUID()) as X1 ) ;
            if ( localent != null )
            {
...
function mapperNewW1 ( ent : W1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedW1(ent, json, localerrors);
}