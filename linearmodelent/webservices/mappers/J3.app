module webservices/mappers/J3
function mapperEditedJ3 ( ent : J3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("K3", obj.getString("id").parseUUID()) as K3 ) ;
            if ( localent != null )
            {
...
function mapperNewJ3 ( ent : J3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedJ3(ent, json, localerrors);
}