module webservices/mappers/R1
function mapperEditedR1 ( ent : R1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("S1", obj.getString("id").parseUUID()) as S1 ) ;
            if ( localent != null )
            {
...
function mapperNewR1 ( ent : R1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedR1(ent, json, localerrors);
}