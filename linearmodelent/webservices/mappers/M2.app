module webservices/mappers/M2
function mapperEditedM2 ( ent : M2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("N2", obj.getString("id").parseUUID()) as N2 ) ;
            if ( localent != null )
            {
...
function mapperNewM2 ( ent : M2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedM2(ent, json, localerrors);
}