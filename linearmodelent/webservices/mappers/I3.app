module webservices/mappers/I3
function mapperEditedI3 ( ent : I3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("J3", obj.getString("id").parseUUID()) as J3 ) ;
            if ( localent != null )
            {
...
function mapperNewI3 ( ent : I3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedI3(ent, json, localerrors);
}