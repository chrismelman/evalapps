module webservices/mappers/X2
function mapperEditedX2 ( ent : X2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("Y2", obj.getString("id").parseUUID()) as Y2 ) ;
            if ( localent != null )
            {
...
function mapperNewX2 ( ent : X2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedX2(ent, json, localerrors);
}