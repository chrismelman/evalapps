module webservices/mappers/R2
function mapperEditedR2 ( ent : R2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("S2", obj.getString("id").parseUUID()) as S2 ) ;
            if ( localent != null )
            {
...
function mapperNewR2 ( ent : R2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedR2(ent, json, localerrors);
}