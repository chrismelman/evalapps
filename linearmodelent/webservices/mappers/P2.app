module webservices/mappers/P2
function mapperEditedP2 ( ent : P2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("Q2", obj.getString("id").parseUUID()) as Q2 ) ;
            if ( localent != null )
            {
...
function mapperNewP2 ( ent : P2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedP2(ent, json, localerrors);
}