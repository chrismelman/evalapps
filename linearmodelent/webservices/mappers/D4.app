module webservices/mappers/D4
function mapperEditedD4 ( ent : D4 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("E4", obj.getString("id").parseUUID()) as E4 ) ;
            if ( localent != null )
            {
...
function mapperNewD4 ( ent : D4 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedD4(ent, json, localerrors);
}