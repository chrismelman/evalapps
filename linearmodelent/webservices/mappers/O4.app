module webservices/mappers/O4
function mapperEditedO4 ( ent : O4 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("P4", obj.getString("id").parseUUID()) as P4 ) ;
            if ( localent != null )
            {
...
function mapperNewO4 ( ent : O4 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedO4(ent, json, localerrors);
}