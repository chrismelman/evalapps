module webservices/mappers/G
function mapperEditedG ( ent : G , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("H", obj.getString("id").parseUUID()) as H ) ;
            if ( localent != null )
            {
...
function mapperNewG ( ent : G , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedG(ent, json, localerrors);
}