module webservices/mappers/Z
function mapperEditedZ ( ent : Z , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("A1", obj.getString("id").parseUUID()) as A1 ) ;
            if ( localent != null )
            {
...
function mapperNewZ ( ent : Z , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedZ(ent, json, localerrors);
}