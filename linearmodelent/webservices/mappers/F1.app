module webservices/mappers/F1
function mapperEditedF1 ( ent : F1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("G1", obj.getString("id").parseUUID()) as G1 ) ;
            if ( localent != null )
            {
...
function mapperNewF1 ( ent : F1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedF1(ent, json, localerrors);
}