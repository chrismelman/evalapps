module webservices/mappers/F2
function mapperEditedF2 ( ent : F2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("G2", obj.getString("id").parseUUID()) as G2 ) ;
            if ( localent != null )
            {
...
function mapperNewF2 ( ent : F2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedF2(ent, json, localerrors);
}