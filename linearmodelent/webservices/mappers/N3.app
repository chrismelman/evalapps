module webservices/mappers/N3
function mapperEditedN3 ( ent : N3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("O3", obj.getString("id").parseUUID()) as O3 ) ;
            if ( localent != null )
            {
...
function mapperNewN3 ( ent : N3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedN3(ent, json, localerrors);
}