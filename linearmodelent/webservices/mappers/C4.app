module webservices/mappers/C4
function mapperEditedC4 ( ent : C4 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("D4", obj.getString("id").parseUUID()) as D4 ) ;
            if ( localent != null )
            {
...
function mapperNewC4 ( ent : C4 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedC4(ent, json, localerrors);
}