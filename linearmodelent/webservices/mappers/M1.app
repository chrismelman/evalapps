module webservices/mappers/M1
function mapperEditedM1 ( ent : M1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("N1", obj.getString("id").parseUUID()) as N1 ) ;
            if ( localent != null )
            {
...
function mapperNewM1 ( ent : M1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedM1(ent, json, localerrors);
}