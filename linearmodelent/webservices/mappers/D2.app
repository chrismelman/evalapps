module webservices/mappers/D2
function mapperEditedD2 ( ent : D2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("E2", obj.getString("id").parseUUID()) as E2 ) ;
            if ( localent != null )
            {
...
function mapperNewD2 ( ent : D2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedD2(ent, json, localerrors);
}