module webservices/mappers/S2
function mapperEditedS2 ( ent : S2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("T2", obj.getString("id").parseUUID()) as T2 ) ;
            if ( localent != null )
            {
...
function mapperNewS2 ( ent : S2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedS2(ent, json, localerrors);
}