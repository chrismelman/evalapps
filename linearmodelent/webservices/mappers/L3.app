module webservices/mappers/L3
function mapperEditedL3 ( ent : L3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("M3", obj.getString("id").parseUUID()) as M3 ) ;
            if ( localent != null )
            {
...
function mapperNewL3 ( ent : L3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedL3(ent, json, localerrors);
}