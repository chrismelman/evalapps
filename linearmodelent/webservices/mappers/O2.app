module webservices/mappers/O2
function mapperEditedO2 ( ent : O2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("P2", obj.getString("id").parseUUID()) as P2 ) ;
            if ( localent != null )
            {
...
function mapperNewO2 ( ent : O2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedO2(ent, json, localerrors);
}