module webservices/mappers/U2
function mapperEditedU2 ( ent : U2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("V2", obj.getString("id").parseUUID()) as V2 ) ;
            if ( localent != null )
            {
...
function mapperNewU2 ( ent : U2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedU2(ent, json, localerrors);
}