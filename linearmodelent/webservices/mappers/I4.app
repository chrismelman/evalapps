module webservices/mappers/I4
function mapperEditedI4 ( ent : I4 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("J4", obj.getString("id").parseUUID()) as J4 ) ;
            if ( localent != null )
            {
...
function mapperNewI4 ( ent : I4 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedI4(ent, json, localerrors);
}