module webservices/mappers/V1
function mapperEditedV1 ( ent : V1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("W1", obj.getString("id").parseUUID()) as W1 ) ;
            if ( localent != null )
            {
...
function mapperNewV1 ( ent : V1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedV1(ent, json, localerrors);
}