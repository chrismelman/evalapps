module webservices/mappers/C1
function mapperEditedC1 ( ent : C1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("D1", obj.getString("id").parseUUID()) as D1 ) ;
            if ( localent != null )
            {
...
function mapperNewC1 ( ent : C1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedC1(ent, json, localerrors);
}