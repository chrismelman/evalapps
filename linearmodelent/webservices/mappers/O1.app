module webservices/mappers/O1
function mapperEditedO1 ( ent : O1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("P1", obj.getString("id").parseUUID()) as P1 ) ;
            if ( localent != null )
            {
...
function mapperNewO1 ( ent : O1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedO1(ent, json, localerrors);
}