module webservices/mappers/I2
function mapperEditedI2 ( ent : I2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("J2", obj.getString("id").parseUUID()) as J2 ) ;
            if ( localent != null )
            {
...
function mapperNewI2 ( ent : I2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedI2(ent, json, localerrors);
}