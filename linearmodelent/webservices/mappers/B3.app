module webservices/mappers/B3
function mapperEditedB3 ( ent : B3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("C3", obj.getString("id").parseUUID()) as C3 ) ;
            if ( localent != null )
            {
...
function mapperNewB3 ( ent : B3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedB3(ent, json, localerrors);
}