module webservices/mappers/T2
function mapperEditedT2 ( ent : T2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("U2", obj.getString("id").parseUUID()) as U2 ) ;
            if ( localent != null )
            {
...
function mapperNewT2 ( ent : T2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedT2(ent, json, localerrors);
}