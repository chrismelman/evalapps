module webservices/mappers/S3
function mapperEditedS3 ( ent : S3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("T3", obj.getString("id").parseUUID()) as T3 ) ;
            if ( localent != null )
            {
...
function mapperNewS3 ( ent : S3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedS3(ent, json, localerrors);
}