module webservices/mappers/N4
function mapperEditedN4 ( ent : N4 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("O4", obj.getString("id").parseUUID()) as O4 ) ;
            if ( localent != null )
            {
...
function mapperNewN4 ( ent : N4 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedN4(ent, json, localerrors);
}