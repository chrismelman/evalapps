module webservices/mappers/H4
function mapperEditedH4 ( ent : H4 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("I4", obj.getString("id").parseUUID()) as I4 ) ;
            if ( localent != null )
            {
...
function mapperNewH4 ( ent : H4 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedH4(ent, json, localerrors);
}