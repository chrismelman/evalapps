module webservices/mappers/O3
function mapperEditedO3 ( ent : O3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("P3", obj.getString("id").parseUUID()) as P3 ) ;
            if ( localent != null )
            {
...
function mapperNewO3 ( ent : O3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedO3(ent, json, localerrors);
}