module webservices/mappers/L1
function mapperEditedL1 ( ent : L1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("M1", obj.getString("id").parseUUID()) as M1 ) ;
            if ( localent != null )
            {
...
function mapperNewL1 ( ent : L1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedL1(ent, json, localerrors);
}