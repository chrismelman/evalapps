module webservices/mappers/D3
function mapperEditedD3 ( ent : D3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("E3", obj.getString("id").parseUUID()) as E3 ) ;
            if ( localent != null )
            {
...
function mapperNewD3 ( ent : D3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedD3(ent, json, localerrors);
}