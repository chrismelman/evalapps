module webservices/mappers/H1
function mapperEditedH1 ( ent : H1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("I1", obj.getString("id").parseUUID()) as I1 ) ;
            if ( localent != null )
            {
...
function mapperNewH1 ( ent : H1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedH1(ent, json, localerrors);
}