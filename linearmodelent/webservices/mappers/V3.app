module webservices/mappers/V3
function mapperEditedV3 ( ent : V3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("W3", obj.getString("id").parseUUID()) as W3 ) ;
            if ( localent != null )
            {
...
function mapperNewV3 ( ent : V3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedV3(ent, json, localerrors);
}