module webservices/mappers/J
function mapperEditedJ ( ent : J , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("K", obj.getString("id").parseUUID()) as K ) ;
            if ( localent != null )
            {
...
function mapperNewJ ( ent : J , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedJ(ent, json, localerrors);
}