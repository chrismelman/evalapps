module webservices/mappers/Y1
function mapperEditedY1 ( ent : Y1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("Z1", obj.getString("id").parseUUID()) as Z1 ) ;
            if ( localent != null )
            {
...
function mapperNewY1 ( ent : Y1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedY1(ent, json, localerrors);
}