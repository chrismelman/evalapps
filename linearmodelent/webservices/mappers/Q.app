module webservices/mappers/Q
function mapperEditedQ ( ent : Q , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("R", obj.getString("id").parseUUID()) as R ) ;
            if ( localent != null )
            {
...
function mapperNewQ ( ent : Q , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedQ(ent, json, localerrors);
}