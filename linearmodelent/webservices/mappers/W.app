module webservices/mappers/W
function mapperEditedW ( ent : W , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("X", obj.getString("id").parseUUID()) as X ) ;
            if ( localent != null )
            {
...
function mapperNewW ( ent : W , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedW(ent, json, localerrors);
}