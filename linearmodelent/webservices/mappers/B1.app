module webservices/mappers/B1
function mapperEditedB1 ( ent : B1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("C1", obj.getString("id").parseUUID()) as C1 ) ;
            if ( localent != null )
            {
...
function mapperNewB1 ( ent : B1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedB1(ent, json, localerrors);
}