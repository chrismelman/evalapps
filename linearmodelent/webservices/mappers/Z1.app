module webservices/mappers/Z1
function mapperEditedZ1 ( ent : Z1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("A2", obj.getString("id").parseUUID()) as A2 ) ;
            if ( localent != null )
            {
...
function mapperNewZ1 ( ent : Z1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedZ1(ent, json, localerrors);
}