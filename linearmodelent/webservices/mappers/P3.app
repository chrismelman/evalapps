module webservices/mappers/P3
function mapperEditedP3 ( ent : P3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("Q3", obj.getString("id").parseUUID()) as Q3 ) ;
            if ( localent != null )
            {
...
function mapperNewP3 ( ent : P3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedP3(ent, json, localerrors);
}