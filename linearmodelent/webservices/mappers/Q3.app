module webservices/mappers/Q3
function mapperEditedQ3 ( ent : Q3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("R3", obj.getString("id").parseUUID()) as R3 ) ;
            if ( localent != null )
            {
...
function mapperNewQ3 ( ent : Q3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedQ3(ent, json, localerrors);
}