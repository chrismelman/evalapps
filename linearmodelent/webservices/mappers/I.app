module webservices/mappers/I
function mapperEditedI ( ent : I , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("J", obj.getString("id").parseUUID()) as J ) ;
            if ( localent != null )
            {
...
function mapperNewI ( ent : I , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedI(ent, json, localerrors);
}