module webservices/mappers/P1
function mapperEditedP1 ( ent : P1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("Q1", obj.getString("id").parseUUID()) as Q1 ) ;
            if ( localent != null )
            {
...
function mapperNewP1 ( ent : P1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedP1(ent, json, localerrors);
}