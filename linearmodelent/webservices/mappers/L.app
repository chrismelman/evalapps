module webservices/mappers/L
function mapperEditedL ( ent : L , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("M", obj.getString("id").parseUUID()) as M ) ;
            if ( localent != null )
            {
...
function mapperNewL ( ent : L , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedL(ent, json, localerrors);
}