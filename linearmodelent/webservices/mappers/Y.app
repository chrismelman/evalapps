module webservices/mappers/Y
function mapperEditedY ( ent : Y , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("Z", obj.getString("id").parseUUID()) as Z ) ;
            if ( localent != null )
            {
...
function mapperNewY ( ent : Y , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedY(ent, json, localerrors);
}