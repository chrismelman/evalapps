module webservices/mappers/N
function mapperEditedN ( ent : N , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("O", obj.getString("id").parseUUID()) as O ) ;
            if ( localent != null )
            {
...
function mapperNewN ( ent : N , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedN(ent, json, localerrors);
}