module webservices/mappers/E1
function mapperEditedE1 ( ent : E1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("F1", obj.getString("id").parseUUID()) as F1 ) ;
            if ( localent != null )
            {
...
function mapperNewE1 ( ent : E1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedE1(ent, json, localerrors);
}