module webservices/mappers/Q2
function mapperEditedQ2 ( ent : Q2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("R2", obj.getString("id").parseUUID()) as R2 ) ;
            if ( localent != null )
            {
...
function mapperNewQ2 ( ent : Q2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedQ2(ent, json, localerrors);
}