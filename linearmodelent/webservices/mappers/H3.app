module webservices/mappers/H3
function mapperEditedH3 ( ent : H3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("I3", obj.getString("id").parseUUID()) as I3 ) ;
            if ( localent != null )
            {
...
function mapperNewH3 ( ent : H3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedH3(ent, json, localerrors);
}