module webservices/mappers/R
function mapperEditedR ( ent : R , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("S", obj.getString("id").parseUUID()) as S ) ;
            if ( localent != null )
            {
...
function mapperNewR ( ent : R , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedR(ent, json, localerrors);
}