module webservices/mappers/S
function mapperEditedS ( ent : S , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("T", obj.getString("id").parseUUID()) as T ) ;
            if ( localent != null )
            {
...
function mapperNewS ( ent : S , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedS(ent, json, localerrors);
}