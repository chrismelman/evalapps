module webservices/mappers/W3
function mapperEditedW3 ( ent : W3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("X3", obj.getString("id").parseUUID()) as X3 ) ;
            if ( localent != null )
            {
...
function mapperNewW3 ( ent : W3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedW3(ent, json, localerrors);
}