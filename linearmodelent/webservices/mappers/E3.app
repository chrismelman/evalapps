module webservices/mappers/E3
function mapperEditedE3 ( ent : E3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("F3", obj.getString("id").parseUUID()) as F3 ) ;
            if ( localent != null )
            {
...
function mapperNewE3 ( ent : E3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedE3(ent, json, localerrors);
}