module webservices/mappers/M
function mapperEditedM ( ent : M , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("N", obj.getString("id").parseUUID()) as N ) ;
            if ( localent != null )
            {
...
function mapperNewM ( ent : M , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedM(ent, json, localerrors);
}