module webservices/mappers/Z2
function mapperEditedZ2 ( ent : Z2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("A3", obj.getString("id").parseUUID()) as A3 ) ;
            if ( localent != null )
            {
...
function mapperNewZ2 ( ent : Z2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedZ2(ent, json, localerrors);
}