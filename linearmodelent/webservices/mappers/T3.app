module webservices/mappers/T3
function mapperEditedT3 ( ent : T3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("U3", obj.getString("id").parseUUID()) as U3 ) ;
            if ( localent != null )
            {
...
function mapperNewT3 ( ent : T3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedT3(ent, json, localerrors);
}