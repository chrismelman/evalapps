module webservices/mappers/J2
function mapperEditedJ2 ( ent : J2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("K2", obj.getString("id").parseUUID()) as K2 ) ;
            if ( localent != null )
            {
...
function mapperNewJ2 ( ent : J2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedJ2(ent, json, localerrors);
}