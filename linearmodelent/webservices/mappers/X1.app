module webservices/mappers/X1
function mapperEditedX1 ( ent : X1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("Y1", obj.getString("id").parseUUID()) as Y1 ) ;
            if ( localent != null )
            {
...
function mapperNewX1 ( ent : X1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedX1(ent, json, localerrors);
}