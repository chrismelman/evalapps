module webservices/mappers/Y3
function mapperEditedY3 ( ent : Y3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("Z3", obj.getString("id").parseUUID()) as Z3 ) ;
            if ( localent != null )
            {
...
function mapperNewY3 ( ent : Y3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedY3(ent, json, localerrors);
}