module webservices/mappers/E4
function mapperEditedE4 ( ent : E4 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("F4", obj.getString("id").parseUUID()) as F4 ) ;
            if ( localent != null )
            {
...
function mapperNewE4 ( ent : E4 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedE4(ent, json, localerrors);
}