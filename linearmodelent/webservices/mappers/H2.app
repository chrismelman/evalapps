module webservices/mappers/H2
function mapperEditedH2 ( ent : H2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("I2", obj.getString("id").parseUUID()) as I2 ) ;
            if ( localent != null )
            {
...
function mapperNewH2 ( ent : H2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedH2(ent, json, localerrors);
}