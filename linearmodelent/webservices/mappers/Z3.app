module webservices/mappers/Z3
function mapperEditedZ3 ( ent : Z3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("A4", obj.getString("id").parseUUID()) as A4 ) ;
            if ( localent != null )
            {
...
function mapperNewZ3 ( ent : Z3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedZ3(ent, json, localerrors);
}