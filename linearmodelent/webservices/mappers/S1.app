module webservices/mappers/S1
function mapperEditedS1 ( ent : S1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("T1", obj.getString("id").parseUUID()) as T1 ) ;
            if ( localent != null )
            {
...
function mapperNewS1 ( ent : S1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedS1(ent, json, localerrors);
}