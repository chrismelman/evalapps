module webservices/mappers/Q1
function mapperEditedQ1 ( ent : Q1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("R1", obj.getString("id").parseUUID()) as R1 ) ;
            if ( localent != null )
            {
...
function mapperNewQ1 ( ent : Q1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedQ1(ent, json, localerrors);
}