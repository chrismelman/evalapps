module webservices/mappers/P4
function mapperEditedP4 ( ent : P4 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("Q4", obj.getString("id").parseUUID()) as Q4 ) ;
            if ( localent != null )
            {
...
function mapperNewP4 ( ent : P4 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedP4(ent, json, localerrors);
}