module webservices/mappers/F3
function mapperEditedF3 ( ent : F3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("G3", obj.getString("id").parseUUID()) as G3 ) ;
            if ( localent != null )
            {
...
function mapperNewF3 ( ent : F3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedF3(ent, json, localerrors);
}