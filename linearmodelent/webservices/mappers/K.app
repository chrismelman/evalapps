module webservices/mappers/K
function mapperEditedK ( ent : K , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("L", obj.getString("id").parseUUID()) as L ) ;
            if ( localent != null )
            {
...
function mapperNewK ( ent : K , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedK(ent, json, localerrors);
}