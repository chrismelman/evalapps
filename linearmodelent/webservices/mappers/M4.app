module webservices/mappers/M4
function mapperEditedM4 ( ent : M4 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("N4", obj.getString("id").parseUUID()) as N4 ) ;
            if ( localent != null )
            {
...
function mapperNewM4 ( ent : M4 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedM4(ent, json, localerrors);
}