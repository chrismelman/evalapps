module webservices/mappers/I1
function mapperEditedI1 ( ent : I1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("J1", obj.getString("id").parseUUID()) as J1 ) ;
            if ( localent != null )
            {
...
function mapperNewI1 ( ent : I1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedI1(ent, json, localerrors);
}