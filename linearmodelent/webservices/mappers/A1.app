module webservices/mappers/A1
function mapperEditedA1 ( ent : A1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("B1", obj.getString("id").parseUUID()) as B1 ) ;
            if ( localent != null )
            {
...
function mapperNewA1 ( ent : A1 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedA1(ent, json, localerrors);
}