module webservices/mappers/M3
function mapperEditedM3 ( ent : M3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("N3", obj.getString("id").parseUUID()) as N3 ) ;
            if ( localent != null )
            {
...
function mapperNewM3 ( ent : M3 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedM3(ent, json, localerrors);
}