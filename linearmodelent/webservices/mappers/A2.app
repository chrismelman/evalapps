module webservices/mappers/A2
function mapperEditedA2 ( ent : A2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("B2", obj.getString("id").parseUUID()) as B2 ) ;
            if ( localent != null )
            {
...
function mapperNewA2 ( ent : A2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedA2(ent, json, localerrors);
}