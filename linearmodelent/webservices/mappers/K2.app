module webservices/mappers/K2
function mapperEditedK2 ( ent : K2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("L2", obj.getString("id").parseUUID()) as L2 ) ;
            if ( localent != null )
            {
...
function mapperNewK2 ( ent : K2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedK2(ent, json, localerrors);
}