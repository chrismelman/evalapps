module webservices/mappers/B2
function mapperEditedB2 ( ent : B2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  if ( ! json.has("list") )
  {
      ...
      for ( count : Int from 0 to temp.length() )
        {
          ...
          if ( temp.get(count) != json.NULL )
          {
            ...
            var localent := ( loadEntity("C2", obj.getString("id").parseUUID()) as C2 ) ;
            if ( localent != null )
            {
...
function mapperNewB2 ( ent : B2 , json : JSONObject , localerrors : JSONArray ) : Void
{
  mapperEditedB2(ent, json, localerrors);
}