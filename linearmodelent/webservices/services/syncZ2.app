module webservices/services/syncZ2
service webservice_generated_syncZ2 ( )
{
  var json := JSONObject() ;
          ...
          for ( innercount : Int from 0 to JSONEntList.length() )
            {
              ...
              {
                for ( ent : Z2 in getAllZ2ForTopEntity(tl) where timestamp == 0 || ent.modified != null && ent.modified.getTime() > timestamp && ent.mayReadSynchronize() )
                  {
  ...
  log("number of entities for syncZ2: " + result.length());
