module webservices/services/syncU1
service webservice_generated_syncU1 ( )
{
  var json := JSONObject() ;
          ...
          for ( innercount : Int from 0 to JSONEntList.length() )
            {
              ...
              {
                for ( ent : U1 in getAllU1ForTopEntity(tl) where timestamp == 0 || ent.modified != null && ent.modified.getTime() > timestamp && ent.mayReadSynchronize() )
                  {
  ...
  log("number of entities for syncU1: " + result.length());
