module webservices/services/syncR
service webservice_generated_syncR ( )
{
  var json := JSONObject() ;
          ...
          for ( innercount : Int from 0 to JSONEntList.length() )
            {
              ...
              {
                for ( ent : R in getAllRForTopEntity(tl) where timestamp == 0 || ent.modified != null && ent.modified.getTime() > timestamp && ent.mayReadSynchronize() )
                  {
  ...
  log("number of entities for syncR: " + result.length());
