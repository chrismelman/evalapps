module webservices/services/syncU3
service webservice_generated_syncU3 ( )
{
  var json := JSONObject() ;
          ...
          for ( innercount : Int from 0 to JSONEntList.length() )
            {
              ...
              {
                for ( ent : U3 in getAllU3ForTopEntity(tl) where timestamp == 0 || ent.modified != null && ent.modified.getTime() > timestamp && ent.mayReadSynchronize() )
                  {
  ...
  log("number of entities for syncU3: " + result.length());
