module webservices/services/syncZ1
service webservice_generated_syncZ1 ( )
{
  var json := JSONObject() ;
          ...
          for ( innercount : Int from 0 to JSONEntList.length() )
            {
              ...
              {
                for ( ent : Z1 in getAllZ1ForTopEntity(tl) where timestamp == 0 || ent.modified != null && ent.modified.getTime() > timestamp && ent.mayReadSynchronize() )
                  {
  ...
  log("number of entities for syncZ1: " + result.length());
